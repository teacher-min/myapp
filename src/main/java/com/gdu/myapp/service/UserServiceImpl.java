...
public class UserServiceImpl implements UserService {
  ...
  public ResponseEntity<Map<String, Object>> checkEmail(Map<String, Object> params) {
    ...
    return ResponseEntity.ok(Map.of("enableEmail", enableEmail));
  }
  ...
  public void signin(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      // 사용자PC -> WAS(톰캣) 상황에서는 getRemoteAddr()를 이용해 사용자 IP를 알 수 있다. 이 때 getHeader("X-Forwarded-For") 값은 null 이다.
      // 사용자PC -> 프록시(로드밸런서 같은) ->  WAS(톰캣) 상황에서는 getRemoteAddr()를 이용해 프록시 주소만 얻을 수 있어서 사용자 IP를 알 수 없다.
      // X-Forwarded-For 값의 첫 번째 주소가 사용자 IP인데 위변조가 가능하여 이걸 그대로 파싱하는 것은 위험하다고 한다. 그래서 스프링에게 맡긴다.
      // 스프링 부트에서는 application.properties 파일에 다음 설정을 추가해서 이 문제를 해결한다.
      ...
      if(user != null) {
        ...
        // session.setMaxInactiveInterval(60 * 30);  // 세션 유지 시간 1800초(30분) 설정 -> application.properties : server.servlet.session.timeout=1800
  ...
  public String getNaverLoginURL(HttpServletRequest request) {
    ...
    String redirectUri = "http://13.124.176.190:8080" + request.getContextPath() + "/user/naver/getAccessToken.do";
